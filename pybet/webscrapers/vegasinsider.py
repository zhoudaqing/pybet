...
class VegasInsider:
    ...
    bases = ['http://www.vegasinsider.com/{}/odds/las-vegas/line-movement',
    ...
    def best_bet(self, team_prediction, wager_type, sportsbooks=None):
        ...
        msg = 'Looking for {} best bets from the following books: {}'
        logger.info(msg.format(team_prediction.team.nickname,
                    ', '.join([s.name for s in sportsbooks])))
        books_offering = []
        for book in sportsbooks:
            curr = book.current_line(team_prediction.team,
                                     team_prediction.date,
                                     wager_type)
            if not curr:
                msg = '{} is not currently offering a line for {}'
                logger.info(msg.format(book.name,
                            team_prediction.team.nickname))
                continue
            else:
                books_offering.append((book, curr))
        if not books_offering:
            return
        best = max(books_offering,
                   key=lambda b: b[1].evaluate(team_prediction))
        book, line = best
        msg = '{} offers the best {} odds for {}'
        logger.info(msg.format(book.name,
                               wager_type,
                               team_prediction.team.nickname))
        return book, line
    ...
    def build_line_history(self, matchups, league):
        ...
        for page in self.bases:
            ...
            for matchup in matchups:
                ...
                msg = 'Retrieving line history for {} @ {}'
                logger.info(msg.format(matchup['away'].nickname,
                            matchup['home'].nickname))
    ...
    def find_sportsbook(self, name):
        ...
        if not best_matches:
            ...
            msg = 'Could not find a sportsbook with the name {}. \
                   Did you mean {}?'.format(name, joined)
        raise KeyError(msg)
