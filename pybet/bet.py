...
class MoneyLine:
    ...
    def __str__(self):
        return '{} ({}) MoneyLine at {}'.format(self.team.nickname,
                                                self.team.league, self.odds)
...
class Spread(MoneyLine):
    ...
    def __str__(self):
        return '{} ({}) against the spread ({}) at {}'.format(
                                                        self.team.nickname,
                                                        self.team.league,
                                                        self.spread,
                                                        self.odds)
...
class Sportsbook:
    ...
    def add_line(self, wager, date):
        if type(wager) == Spread:
            ...
            logger.debug('Added spread wager for {}: ({}) to {}'.format(
                                                        wager.team.nickname,
                                                        id(wager.team),
                                                        self.name))
        elif type(wager) == MoneyLine:
            self.moneyline_history[(wager.team, date)].append(wager)
            logger.debug('Added moneyline wager for {}: ({}) to {}'.format(
                                                        wager.team.nickname,
                                                        id(wager.team),
                                                        self.name))
        else:
